...
class Fmbench(BaseDataset):
    ...
    def _format_data(self, data):
        ...
        desc_file = os.path.join(sub_dump_folder, dump_name + '_' + filename + 'descriptors')
        kpt_file = os.path.join(sub_dump_folder, dump_name + '_' + filename + 'keypoints')
        ...
        if len(dump_path) > 1:
            for i in range(1, len(dump_path)):
                ...
                same_desc_file = os.path.join(sub_dump_folder, dump_name + '_' + same_filename + '.descriptors')
                same_kpt_file = os.path.join(sub_dump_folder, dump_name + '_' + same_filename + '.keypoints')
