...
class SuperpointFbm(object):
    ...
    def __init__(self,  **config):
        ...
        self.model = SuperPointDetector().to(self.device)
        if self.config["dim"] == 128:
            print("Initialize Extractor 128")
            self.extractor = SuperPointExtractor().to(self.device)
        elif self.config["dim"] == 256:
            print("Initialize Extractor 256")
    ...
    def predict(self, img, keys="*"):
        ...
        # detector
        _, prob, c1, c2, c3, c4 = self.model(img)
