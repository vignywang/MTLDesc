...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        # self.conv_avg = ConvBNReLU(2048, 128, ks=1, stride=1, padding=0)
...
class SpatialPathMultiFeat(nn.Module):
    def __init__(self, *args, **kwargs):
        super(SpatialPathMultiFeat, self).__init__()
...
class BiSeNetV1Point(nn.Module):
    ...
    def forward(self, x):
        H, W = x.size()[2:]
        feat_cp8, feat_cp16 = self.cp(x)
        heatmap, c1, c2, c3, c4 = self.sp(x)
        feat_fuse = self.ffm(c4, feat_cp8)
...
if __name__ == "__main__":
    ...
    net.cuda()
    net.eval()
    in_ten = torch.randn(1, 3, 513, 513).cuda()
    out = net(in_ten)
    print(out.shape)
