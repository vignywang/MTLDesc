...
class MegpointSegmentationTrainer(MegPointTrainer):

    def __init__(self, **config):
        super(MegpointSegmentationTrainer, self).__init__(**config)
    ...
    def _train_func(self, epoch_idx):
        ...
        for i, data in enumerate(self.train_dataloader):
            ...
            # 模型预测
            heatmap_pred_pair, c1_pair, c2_pair, c3_pair, c4_pair = self.model(image_pair)
            seg_pair = self.segmentor(image_seg_pair)
            ...
            c3_feature_pair = f.grid_sample(c3_pair, desp_point_pair, mode="bilinear", padding_mode="border")
            c4_feature_pair = f.grid_sample(c4_pair, desp_point_pair, mode="bilinear", padding_mode="border")
            seg_feature_pair = f.grid_sample(seg_pair, desp_point_pair, mode='bilinear', padding_mode='border')

            feature_pair = torch.cat(
                (c1_feature_pair, c2_feature_pair, c3_feature_pair, c4_feature_pair, seg_feature_pair), dim=1)
            feature_pair = feature_pair[:, :, :, 0].transpose(1, 2)
            desp_pair = self.extractor(feature_pair)
            desp_0, desp_1 = torch.chunk(desp_pair, 2, dim=0)
    ...
    def _inference_func(self, image_pair):
        ...
        self.extractor.eval()
        _, _, height, width = image_pair.shape
        seg_image_pair = (image_pair + 1) * 255. / 2. - torch.tensor((122.675, 116.669, 104.008), device=self.device).view((1, 3, 1, 1))

        heatmap_pair, c1_pair, c2_pair, c3_pair, c4_pair = self.model(image_pair)
        seg_pair = self.segmentor(seg_image_pair)
    ...
    def _generate_combined_descriptor_fast(self, point, c1, c2, c3, c4, seg, height, width):
        ...
        c3_feature = f.grid_sample(c3, point, mode="bilinear")[:, :, :, 0].transpose(1, 2)
        c4_feature = f.grid_sample(c4, point, mode="bilinear")[:, :, :, 0].transpose(1, 2)
        seg_feature = f.grid_sample(seg, point, mode='bilinear')[:, :, :, 0].transpose(1, 2)

        feature = torch.cat((c1_feature, c2_feature, c3_feature, c4_feature, seg_feature), dim=2)
        desp = self.extractor(feature)[0]  # [n,128]
