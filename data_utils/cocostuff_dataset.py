...
class CocoStuff10kTrainPoint(Dataset):
    ...
    def __getitem__(self, idx):
        image_path = osp.join(self.config['root'], 'processed_dataset', self.file_list[idx] + ".jpg")
        label_path = osp.join(self.config['root'], 'processed_dataset', self.file_list[idx] + "_label.npy")
        point_path = osp.join(self.config['root'], 'processed_dataset', self.file_list[idx] + '_point.npy')
        ...
        # preprocess
        # image, label, point = self.preprocess(image, label, point)

        # sample homography
        point_mask = np.ones_like(image).astype(np.float32)[:, :, 0].copy()
        if torch.rand([]).item() < 0.5:
            warped_image, warped_label, warped_point_mask, warped_point, homography = \
                image.copy(), label.copy(), point_mask.copy(), point.copy(), np.eye(3)
        else:
        ...
        # heatmap for the original image
        heatmap = self._convert_points_to_heatmap(point)

        # heatmap for the warped image
    ...
    def preprocess(self, image, label, point):
        h, w = label.shape
        # first resize
        if max(h, w) > 320:
            s = float(max(h, w)) / 320.
            image = cv2.resize(image, None, None, fx=1 / s, fy=1 / s, interpolation=cv2.INTER_LINEAR)
            label = Image.fromarray(label).resize((int(w/s), int(h/s)), resample=Image.NEAREST)
        ...
        if pad_h > 0 or pad_w > 0:
            image = cv2.copyMakeBorder(image, value=0, **pad_kwargs)
            label = cv2.copyMakeBorder(label, value=0, **pad_kwargs)
            point = point + np.array((pad_h, pad_w))
    ...
    def _format_file_list(self):
        file_list = osp.join(self.config['root'], "imageLists",  "train.txt")
        file_list = tuple(open(file_list, "r"))
        file_list = [id_.rstrip() for id_ in file_list]
