...
class MegaDepthCOCODataset(Dataset):
    """
    ...
    @staticmethod
    def _format_file_list(coco_dataset_dir, mega_dataset_dir, mega_label_dir):
        data_list = []
        ...
        for img in mega_image_list:
            ...
            info = img[:-3] + 'npz'
            label = os.path.join(mega_label_dir, 'train', img_name + '.npz')
            data_list.append(
                {
                    'type': mega_type,
                    'image': img,
        ...
        coco_type = 'coco'
        coco_image_root = os.path.join(coco_dataset_dir, "images")
        coco_point_root = os.path.join(coco_dataset_dir, 'pseudo_image_points_1')
        org_image_list = glob(os.path.join(coco_image_root, "*.jpg"))
        ...
        for image_dir in org_image_list:
            ...
            point_dir = os.path.join(coco_point_root, name + '.npy')
            try:
