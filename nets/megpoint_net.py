...
class ResNetAll(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.compress11 = nn.Conv2d(64, 16, kernel_size=1, stride=1)
        self.compress22 = nn.Conv2d(128, 16, kernel_size=1, stride=1)
        self.compress33 = nn.Conv2d(256, 16, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        compress11 = self.compress11(c1)
        compress22 = self.compress22(c2)
...
def _resnet_all(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNetAll(block, layers, **kwargs)
    return model


def resnet18_all(pretrained=False, progress=True, **kwargs):
    return _resnet_all('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress, **kwargs)


class HalfResNetAll(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(HalfResNetAll, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.compress1 = nn.Conv2d(32, 32, kernel_size=1, stride=1)
        self.compress2 = nn.Conv2d(64, 32, kernel_size=1, stride=1)
        self.compress3 = nn.Conv2d(128, 32, kernel_size=1, stride=1)
        self.compress4 = nn.Conv2d(256, 32, kernel_size=1, stride=1)

        self.compress11 = nn.Conv2d(32, 16, kernel_size=1, stride=1)
        self.compress22 = nn.Conv2d(64, 16, kernel_size=1, stride=1)
        self.compress33 = nn.Conv2d(128, 16, kernel_size=1, stride=1)
        self.compress44 = nn.Conv2d(256, 16, kernel_size=1, stride=1)
...
def _half_resnet_all(arch, block, layers, pretrained, progress, **kwargs):
    model = HalfResNetAll(block, layers, **kwargs)
    return model


def half_resnet18_all(pretrained=False, progress=True, **kwargs):
    return _half_resnet_all('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress, **kwargs)
