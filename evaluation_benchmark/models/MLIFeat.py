...
from nets.MLIFeat_net import SuperPointExtractorNoL2


class Mlifeat(object):

    def __init__(self,  **config):
        self.name = 'MLIFeatNoL2'
        self.config = {
        ...
        # 初始化resnet18_fast
        print("Initialize MLIFeat")
        self.model = SuperPointNetBackbone().to(self.device)
        if self.config["l2"]:
            print("Initialize MLIFeat 128")
            self.extractor = SuperPointExtractor().to(self.device)
        else:
            print('Initialize MLIFeat 128 no l2')
            self.extractor = SuperPointExtractorNoL2().to(self.device)
    ...
    def size(self):
        ...
        summary(self.extractor, input_size=(1, 384))
        # _, params1 = profile(self.model, rand_input1)
        # _, params2 = profile(self.extractor, rand_input2)
        # return size1 + size2
...
class HPatchMLIFeat(Mlifeat):
    """专用于hpatch测试的SuperPoint模型"""

    def __init__(self, ckpt_root):
        super(HPatchMLIFeat, self).__init__()
        self.load(ckpt_root)
