...
def resnet18_all():
    return ResNetAll(BasicBlock, [2, 2, 2, 2])
...
class ResNetFast(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.heatmap = nn.Conv2d((16 + 8 + 4 + 2), 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        heatmap1 = f.pixel_shuffle(c1, 2)
        heatmap2 = f.pixel_shuffle(c2, 4)
        heatmap3 = f.pixel_shuffle(c3, 8)
        heatmap4 = f.pixel_shuffle(c4, 16)
...
def resnet18_fast():
    return ResNetFast(BasicBlock, [2, 2, 2, 2])


class ResNetFastS4(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNetFastS4, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
def resnet18_fast_s4():
    return ResNetFastS4(BasicBlock, [2, 2, 2, 2])


class ResNetFastS3S4(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNetFastS3S4, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.heatmap = nn.Conv2d((4 + 2), 1, kernel_size=3, stride=1, padding=1)
...
def resnet18_fast_s3s4():
    return ResNetFastS3S4(BasicBlock, [2, 2, 2, 2])


class ResNetFastS2S3S4(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNetFastS2S3S4, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.heatmap = nn.Conv2d((8 + 4 + 2), 1, kernel_size=3, stride=1, padding=1)
...
def resnet18_fast_s2s3s4():
    return ResNetFastS2S3S4(BasicBlock, [2, 2, 2, 2])
