...
class ResNetSuperPointHeadC4(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNetSuperPointHeadC4, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        # Descriptor Head.
        self.convDa = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
...
def resnet18_superpoint_head_c4():
    return ResNetSuperPointHeadC4(BasicBlock, [2, 2, 2, 2])
