...
from utils.utils import DistillVectorL1Loss
...
class MegpointDistill1(MegPointTrainer):

    def __init__(self, **config):
        super(MegpointDistill1, self).__init__(**config)
    ...
    def _initialize_loss(self):
        ...
        # 初始化蒸馏loss
        self.logger.info('Initialize the Distillation loss')
    ...
    def _initialize_model(self):
        ...
        segmentor = get_model(self.config['model']['segmentor'])(**self.config['model'])

        if self.multi_gpus:
            model = torch.nn.DataParallel(model)
            extractor = torch.nn.DataParallel(extractor)
            segmentor = torch.nn.DataParallel(segmentor)
        self.model = model.to(self.device)
        self.extractor = extractor.to(self.device)
        self.segmentor = segmentor.to(self.device)

        # restore segmentor ckpt
    ...
    def _train_func(self, epoch_idx):
        ...
        for i, data in enumerate(self.train_dataloader):
            ...
            point_mask = data['point_mask'].to(self.device)
            desp_point = data["desp_point"].to(self.device)
            image_seg = data['image_seg'].to(self.device)

            warped_image = data["warped_image"].to(self.device)
            warped_heatmap_gt = data['warped_heatmap'].to(self.device)
            warped_point_mask = data['warped_point_mask'].to(self.device)
            warped_desp_point = data["warped_desp_point"].to(self.device)
            warped_image_seg = data['warped_image_seg'].to(self.device)

            valid_mask = data["valid_mask"].to(self.device)
            not_search_mask = data["not_search_mask"].to(self.device)

            image_pair = torch.cat((image, warped_image), dim=0)
            image_seg_pair = torch.cat((image_seg, warped_image_seg), dim=0)

            # 模型预测
            heatmap_pred_pair, c1_pair, c2_pair, c3_pair, c4_pair, seg_logit_pair = self.model(image_pair)
            tearcher_seg_logit_pair = self.segmentor(image_seg_pair)
            tearcher_seg_logit_pair = f.interpolate(
                tearcher_seg_logit_pair, size=[seg_logit_pair.shape[2], seg_logit_pair.shape[3]], mode='bilinear', align_corners=True)

            # 计算蒸馏loss
            seg_loss = self.distill_loss(seg_logit_pair, tearcher_seg_logit_pair)
            ...
            loss = desp_loss + 0.1 * point_loss + 0.1 * seg_loss
            ...
            if i % self.config['train']['log_freq'] == 0:
                ...
                self.logger.info(
                    "[Epoch:%2d][Step:%5d:%5d]: loss = %.4f, point_loss = %.4f, desp_loss = %.4f, seg_loss = %.4f"
                    " one step cost %.4fs. " % (
                        epoch_idx, i, self.epoch_length,
                        loss_val,
                        point_loss_val,
        ...
        if self.multi_gpus:
            torch.save(
                self.model.module.state_dict(), os.path.join(self.config['ckpt_path'], 'model.pt'))
            torch.save(
                self.extractor.module.state_dict(), os.path.join(self.config['ckpt_path'], 'extractor.pt'))
        else:
            torch.save(
                self.model.state_dict(), os.path.join(self.config['ckpt_path'], 'model.pt'))
            torch.save(
                self.extractor.state_dict(), os.path.join(self.config['ckpt_path'], 'extractor.pt'))
...
class MegpointDistill2(MegPointTrainer):

    def __init__(self, **config):
        super(MegpointDistill2, self).__init__(**config)
    ...
    def _initialize_loss(self):
        ...
        # 初始化蒸馏loss
        self.logger.info('Initialize the Distillation loss')
        self.distill_loss = DistillVectorL1Loss()
    ...
    def _train_func(self, epoch_idx):
        ...
        for i, data in enumerate(self.train_dataloader):
            ...
            # 模型预测
            heatmap_pred_pair, c1_pair, c2_pair, c3_pair, c4_pair = self.model(image_pair)
            teacher_seg_logit_map = self.segmentor(image_seg_pair)
            ...
            c3_feature_pair = f.grid_sample(c3_pair, desp_point_pair, mode="bilinear", padding_mode="border")
            c4_feature_pair = f.grid_sample(c4_pair, desp_point_pair, mode="bilinear", padding_mode="border")
            teacher_seg_logit_pair = f.grid_sample(teacher_seg_logit_map, desp_point_pair, mode='bilinear', padding_mode='border')
            teacher_seg_logit_pair = teacher_seg_logit_pair[:, :, :, 0].transpose(1, 2)

            feature_pair = torch.cat((c1_feature_pair, c2_feature_pair, c3_feature_pair, c4_feature_pair), dim=1)
            feature_pair = feature_pair[:, :, :, 0].transpose(1, 2)
            desp_pair, seg_logit_pair = self.extractor(feature_pair)
            desp_0, desp_1 = torch.chunk(desp_pair, 2, dim=0)

            desp_loss = self.descriptor_loss(desp_0, desp_1, valid_mask, not_search_mask)

            # 计算蒸馏loss
            seg_loss = self.distill_loss(
                seg_logit_pair,
                teacher_seg_logit_pair,
                torch.cat((torch.ones_like(valid_mask), valid_mask), dim=0)
            )
            ...
            loss = desp_loss + 0.1 * point_loss + 0.01 * seg_loss
...
class SuperPointFBMTrainer(MegPointTrainer):

    def __init__(self, **config):
        super(SuperPointFBMTrainer, self).__init__(**config)
    ...
    def _initialize_optimizer(self):
        ...
        self.optimizer = torch.optim.Adam(
            ...
            lr=self.config['train']['lr'],
            weight_decay=self.config['train']['weight_decay'])
        self.extractor_optimizer = torch.optim.Adam(params=self.extractor.parameters(),
                                                    lr=self.config['train']['lr'],
                                                    weight_decay=self.config['train']['weight_decay'])
...
class SuperPointFSMTrainer(MegPointTrainer):

    def __init__(self, **config):
        super(SuperPointFSMTrainer, self).__init__(**config)
