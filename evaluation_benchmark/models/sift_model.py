...
class BasicSIFT(object):
    ...
    def __init__(self, logger, top_k=1000):
        ...
        # 初始化sift
        self.logger.info("Initialize sift")
        self.sfit = cv.xfeatures2d_SIFT.create(nfeatures=top_k)
    ...
    def generate_feature(self, img):
        ...
        """
        # detect and compute
        keypoints_cv, desp = self.sfit.detectAndCompute(img, None)
...
class HPatchSIFT(BasicSIFT):
    """专用于hpatch测试的SuperPoint模型"""

    def __init__(self, logger, top_k=1000):
        super(HPatchSIFT, self).__init__(logger, top_k=top_k)
