...
from nets.deeplab_net import DeeplabV2


class MegpointDeeplabv2(object):
    ...
    def __init__(self,  **config):
        ...
        # 初始化segmentation模型
        print('Initialize Deeplabv2')
        self.segmentor = DeeplabV2(**self.config).to(self.device)
    ...
    def load(self, checkpoint_root):
        ...
        segmentation_ckpt = os.path.join(checkpoint_root, 'deeplabv2.pt')
    ...
    def predict(self, img, keys="*"):
        ...
        if scale_h > 1000 or scale_w > 1000:
            seg_img = cv.resize(seg_img, None, None, fx=0.5, fy=0.5, interpolation=cv.INTER_LINEAR)
        seg_img = seg_img - self.mean_bgr
