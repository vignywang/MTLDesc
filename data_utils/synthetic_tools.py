...
""" 合成具有简单形状以及interest point 的图片以供 magicpoint 训练 """

random_state = np.random.RandomState(None)    # 随机数种子
...
def get_random_color(background_color):
    """ 生成与背景颜色有差异的随机颜色用作合成图像中前景的颜色 """
    color = random_state.randint(256)
    if abs(color - background_color) < 30:  # 与背景颜色差异过小，放大差异
        color = (color + 128) % 256
    return color


def get_different_color(previous_colors, min_dist=50, max_count=20):
    """ 生成与之前的所有前景颜色具有一定差异的新颜色
    Parameters:
      previous_colors: 含有所有之前前景颜色的矩阵
      min_dist: 最小要求的颜色差异
      max_count: 最大尝试次数，控制运行时间
    """
    color = random_state.randint(256)
    count = 0
...
def add_salt_and_pepper(img):
    ...
    cv.randu(noise, 0, 255)  # 0-255之间的均匀分布矩阵，来决定椒盐噪声的位子
    black = noise < 30
    white = noise > 225
    img[white > 0] = 255  # 椒
    img[black > 0] = 0  # 盐
...
def generate_background(size=(960, 1280), nb_blobs=100, min_rad_ratio=0.01,
    ...
    Parameters:
      size: 图像尺寸
      nb_blobs: 画圆的数目
      min_rad_ratio: 圆形的最小比（与图像尺寸的较大值相比）
      max_rad_ratio: 圆形的最大比（与图像尺寸的较大值相比）
      min_kernel_size: 模糊核的最小值
      max_kernel_size: 模糊核的最大值
    """
    img = np.zeros(size, dtype=np.uint8)
    dim = max(size)
    cv.randu(img, 0, 255)
    cv.threshold(img, random_state.randint(256), 255, cv.THRESH_BINARY, img)  # 用随机阈值对图像进行二值化
    ...
    blobs = np.concatenate([random_state.randint(0, size[1], size=(nb_blobs, 1)),
                           ...
                           axis=1)  # 生成数量大小为 nb_blobs 的随机位置矩阵，用来确定要画圆的位置
    for i in range(nb_blobs):
        ...
        cv.circle(img, (blobs[i][0], blobs[i][1]),
                  ...
                  col, -1)  # 根据对圆形大小的限制和之前随机生成的位置来作圆
    kernel_size = random_state.randint(min_kernel_size, max_kernel_size)
    cv.blur(img, (kernel_size, kernel_size), img)  # 模糊整幅图像
    return img
...
def generate_custom_background(size, background_color, nb_blobs=3000,
    ...
    Parameters:
      background_color: 背景图平均像素值
      nb_blobs: 画圆个数
      kernel_boundaries: 模糊核的大小范围
    """
    img = np.zeros(size, dtype=np.uint8)
