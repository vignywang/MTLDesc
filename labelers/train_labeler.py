...
class TrainDatasetCreator(object):
       ...
       # cur_time = time.time()
        ...
        for i, scene in enumerate(self.scenes):
            ...
            for pair_idx in tqdm(selected_ids):
                ...
                image1, image2, desp_point1, desp_point2, valid_mask, not_search_mask,keypint1,keypint2,raw_desp_point1,raw_desp_point2 = self.get_pair(
                ...
                np.savez(cur_key_path, points_0=keypint1, points_1=keypint2)
    ...
    def get_pair(
        ...
        elif self.config['despoint_type'] == 'heatmapleft':
            # 只是用左图关键点作为训练描述子采样点
            desp_point1 =self._tensor_NMStop(tensor_heatmap1)
        else:
            # 使用交叉验证heatmap采样描述子
            tensor_heatmap2 = self._generate_warped_heatmap(self.all_points,depth2, bbox2, pose2,intrinsics2, depth1, bbox1, pose1,intrinsics1,tensor_heatmap2)
            heatmap=tensor_heatmap1+tensor_heatmap2
            desp_point1=self._NMStop(heatmap)
        desp_point2, valid_mask, not_search_mask = self._generate_warped_point(desp_point1, depth1, bbox1, pose1, intrinsics1, depth2, bbox2, pose2, intrinsics2)
        scale_desp_point1 = self._scale_point_for_sample(desp_point1, height=self.image_height, width=self.image_width)
        scale_desp_point2 = self._scale_point_for_sample(desp_point2, height=self.image_height, width=self.image_width)

        return image1, image2, scale_desp_point1, scale_desp_point2, valid_mask, not_search_mask, keypint1, keypint2,desp_point1,desp_point2
    ...
    def _generate_warped_heatmap(
        ...
        src_point = np.concatenate((src_point, np.ones((src_point.shape[0], 1), np.float32)), axis=1)[:, :,
                    np.newaxis]  # [n,3,1]
      ...
      #  invalid_mask = ~valid_mask
       ...
       # heatmap1[src_point]+=heatmap2[tgt_point]
    ...
    @staticmethod
    def interpolate_depth2(pos, depth):
        device = pos.device
    ...
    def warp(self,
            pos1,
            depth1, bbox1, pose1, intrinsics1,
            depth2, bbox2, pose2, intrinsics2,
            heatmap2):

        device = self.device
        pos1= torch.from_numpy(pos1).cuda(self.device)
        depth1=torch.from_numpy(depth1).cuda(self.device)
        bbox1=torch.from_numpy(bbox1).cuda(self.device)
        pose1=torch.from_numpy(pose1).cuda(self.device)
        intrinsics1=torch.from_numpy(intrinsics1).cuda(self.device)
        depth2=torch.from_numpy(depth2).cuda(self.device)
        bbox2=torch.from_numpy(bbox2).cuda(self.device)
        pose2=torch.from_numpy(pose2).cuda(self.device)
        intrinsics2=torch.from_numpy(intrinsics2).cuda(self.device)

        Z1, pos1, ids = self.interpolate_depth2(pos1, depth1)
        ...
        annotated_depth, pos2, new_ids =  self.interpolate_depth2( self.uv_to_pos(uv2), depth2)
